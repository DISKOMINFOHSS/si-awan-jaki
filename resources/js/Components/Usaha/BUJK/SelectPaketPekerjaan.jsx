...
export default ({ isVisible, onSelect, daftarPaketPekerjaan }) => {
    ...
    return (
        <div className= "absolute flex z-20 bg-white mt-1.5 py-2 rounded shadow-xl min-w-full max-h-40 overflow-y-auto flex-col space-y-0.5 text-xs text-slate-700">
            <div className="px-4 py-2 border-b border-slate-100">
                <input
                    type="text" value={keyword} onChange={handleKeywordChange} placeholder="Cari Paket Pekerjaan"
            ...
            </div>
            {
                filteredDaftarPaketPekerjaan.map((paketPekerjaan) => (
                    <div
                        ...
                        className="px-5 py-2 border-b border-slate-100 group hover:bg-slate-100 hover:text-blue-600"
                        onClick={() => onSelect(paketPekerjaan)}
                    >
                        <div>
                            <span>{paketPekerjaan.namaPaket} - </span>
                            <span className="font-light text-slate-500 group-hover:text-blue-600">TA. {paketPekerjaan.tahunAnggaran}</span>
                        </div>
                    </div>
                ))
            }
