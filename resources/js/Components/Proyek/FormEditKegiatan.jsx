...
function FormInformasi({ proyekKonstruksi }) {
    ...
    const { data, setData, put, processing, reset } = useForm({
        namaPaket: getDefaultData(namaPaket),
        nomorKontrak: getDefaultData(nomorKontrak),
        sumberDana: getDefaultData(sumberDana, 'APBD'),
        tanggalMulai: getDefaultData(tanggalMulaiPelaksanaan),
        tanggalSelesai: getDefaultData(tanggalSelesaiPelaksanaan),
        tanggalKontrak: getDefaultData(tanggalKontrak),
...
function FormPenyediaJasa({ proyekId, selectedUsaha, daftarUsaha }) {
    const { data, setData, post, processing, reset } = useForm({
        usaha: getFormData(selectedUsaha.nama),
        usahaId: selectedUsaha.id,
        nib: getFormData(selectedUsaha.nib),
        dokumenNIB: selectedUsaha.fileId ? {
            fileId: selectedUsaha.fileId,
            fileName: selectedUsaha.fileName,
            filePath: selectedUsaha.filePath,
        } : '',
        pjbu: getFormData(selectedUsaha.pjbu),
        alamat: getFormData(selectedUsaha.alamat),
    });
    ...
    const inputClass = classNames(
        "px-3 py-2 block w-full rounded-md border-slate-200 text-slate-600 placeholder:text-slate-500 focus:ring-blue-400 focus:border-blue-400 text-xs",
        {
            "bg-slate-50": selectedUsaha,
        }
    );
    ...
    function handleSubmit(e) {
        e.preventDefault();
        console.log(data);
        post(`/admin/pendataan/proyek-konstruksi/${proyekId}/penyedia-jasa`, {
            preserveScroll: true,
            onSuccess: () => {
                setIsModalSuccessOpen(true);
            },
    ...
    return (
        <>
                    ...
                    >
                        ...
                        <div className="col-span-2">
                            ...
                            <input
                                ...
                                value={data.nib} onChange={e => setData('nib', e.target.value)} disabled={selectedUsaha}
                        ...
                        <div className="col-span-2">
                            ...
                            <input
                                ...
                                value={data.pjbu} onChange={e => setData('pjbu', e.target.value)} disabled={selectedUsaha}
                                className={inputClass}
                            />
                        </div>
                        <div className="col-span-3">
                            ...
                            <textarea
                                ...
                                value={data.alamat} onChange={e => setData('alamat', e.target.value)} disabled={selectedUsaha}
                                className={inputClass}
                            />
                        </div>
            ...
            >
                <div className="font-medium text-slate-700 mb-1">Uh Oh!</div>
                <div className="font-light text-xs text-slate-500 mb-2">
                    Gagal menambahkan penyedia jasa pada proyek konstruksi ini. Silakan periksa kembali informasi yang diisi.
                </div>
            </ModalError>
            <ModalSuccess
                isVisible={isModalSuccessOpen}
...
function FormPenggunaJasa({ proyekId, penggunaJasa }) {
    const { data, setData, post, processing, reset } = useForm({
        penggunaJasaId: penggunaJasa.id,
        nama: getFormData(penggunaJasa.nama),
        pelakuPengadaan: getFormData(penggunaJasa.pelakuPengadaan, 'Kuasa Pengguna Anggaran'),
        nip: getFormData(penggunaJasa.nil),
        jabatan: getFormData(penggunaJasa.jabatan),
        sk: getFormData(penggunaJasa.sk),
        instansi: getFormData(penggunaJasa.instansi),
        alamat: getFormData(penggunaJasa.alamat),
    });
    ...
    function handleSubmit(e) {
        e.preventDefault();
        console.log(data);
        post(`/admin/pendataan/proyek-konstruksi/${proyekId}/pengguna-jasa`, {
            preserveScroll: true,
            onSuccess: () => {
                setIsModalSuccessOpen(true);
            },
    ...
    return (
        <>
            ...
            >
                <div className="font-medium text-slate-700 mb-1">Uh Oh!</div>
                <div className="font-light text-xs text-slate-500 mb-2">
                    Gagal menambahkan pengguna jasa pada proyek konstruksi ini. Silakan periksa kembali informasi yang diisi.
                </div>
            </ModalError>
            <ModalSuccess
                isVisible={isModalSuccessOpen}
...
function FormKonsultanPengawas({ proyekId, selectedUsaha, daftarUsaha, namaPaket }) {
    const { data, setData, post, processing, reset } = useForm({
        usaha: getDefaultData(selectedUsaha.nama),
        usahaId: selectedUsaha.id,
        nib: getDefaultData(selectedUsaha.nib),
        dokumenNIB: selectedUsaha.fileId ? {
            fileId: selectedUsaha.fileId,
            fileName: selectedUsaha.fileName,
            filePath: selectedUsaha.filePath,
        } : '',
        pjbu: getDefaultData(selectedUsaha.pjbu),
        alamat: getDefaultData(selectedUsaha.alamat),
        namaPaket: getDefaultData(namaPaket),
    });
    ...
    function handleSubmit(e) {
        e.preventDefault();
        console.log(data);
        post(`/admin/pendataan/proyek-konstruksi/${proyekId}/konsultan-pengawas`, {
            preserveScroll: true,
            onSuccess: () => {
                setIsModalSuccessOpen(true);
            },
    ...
    return (
        <>
                ...
                <Card.Body className="p-4">
                    ...
                    >
                        ...
                        <div className="col-span-5">
                            <label htmlFor="paketPengawasan" className="block mb-2 text-xs font-medium text-slate-800">Nama Paket Pekerjaan <span className="text-red-400">*</span></label>
                            <textarea
                                name="paketPengawasan" id="paketPengawasan" rows="2" placeholder="cth. Jasa Konsultansi Pengawasan Perkuatan Tebing di Desa Angkinang Kec. Angkinang"
                                value={data.namaPaket} onChange={e => setData('namaPaket', e.target.value)}
                                className="px-3 py-2 block w-full rounded-md border-slate-200 text-slate-600 placeholder:text-slate-500 focus:ring-blue-400 focus:border-blue-400 text-xs"
                            />
                        </div>
            ...
            >
                <div className="font-medium text-slate-700 mb-1">Uh Oh!</div>
                <div className="font-light text-xs text-slate-500 mb-2">
                    Gagal menambahkan Konsultan Pengawas pada proyek konstruksi ini. Silakan periksa kembali informasi yang diisi.
                </div>
            </ModalError>
            <ModalSuccess
                isVisible={isModalSuccessOpen}
            ...
            >
                <div className="text-center my-2.5">
                    <div className="font-medium text-slate-700">Berhasil!</div>
                    <div className="font-light text-xs text-slate-500 mb-2">
                        Konsultan Pengawas berhasil ditambahkan.
                    </div>
                </div>
                <div className="w-full">
                    <button
