                    ...
                    <table className="w-full text-xs">
                        <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase">
                            <tr className="border-b border-slate-200">
                                ...
                                <th scope="col" className="p-4 font-medium min-w-60 border-r border-slate-200">Nama Usaha Orang Perseorangan</th>
                                ...
                                filteredDaftarPengawasan.map((pengawasan, i) => (
                                    <tr key={pengawasan.id} className="border-b border-slate-100 hover:bg-slate-50">
                                        ...
                                        <td className="px-4 py-5">
                                            <div>
                                                <Link href={`/admin/pengawasan/usaha/4/usaha-perseorangan/${pengawasan.id}`} className="uppercase hover:text-blue-600 hover:underline">
                                        ...
                                        <td className="px-4 py-5 text-center">
                                            {
                                                pengawasan.usaha.skk.map(({ nomor_sertifikat }, i) => {
                                                    if (pengawasan.usaha.skk.length === i + 1) return nomor_sertifikat;
                                                    return `${nomor_sertifikat} , `;
                                                })
                                            }
                                        </td>
                                        <td className="px-4 py-5 text-center">{getDefaultData(pengawasan.usaha.alamat, '-')}</td>
