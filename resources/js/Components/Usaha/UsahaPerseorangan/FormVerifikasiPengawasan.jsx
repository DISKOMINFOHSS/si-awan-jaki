...
export default ({
    isVisible, onClose, lingkupPengawasan, pengawasan
}) => {
    const {
        tertibPersyaratanSKK,
    ...
    const [ values, setValues ] = React.useState({
        syaratSKK: tertibPersyaratanSKK,
    ...
    function handleSubmit(e) {
        ...
        setProcessing(true);
        router.post(
            `/admin/pengawasan/usaha/${lingkupPengawasan.id}/usaha-perseorangan/${pengawasan.id}/verification`,
            values,
            {
                onSuccess: () => {
                    onClose();
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 border-b border-slate-100 pb-4">
                            <div className="space-y-2 text-xs">
                                <div>
                                    <div className="text-slate-800">Sertifikat Kompetensi Kerja (SKK) <span className="text-red-400">*</span></div>
                                    <div className="font-light text-[11px] text-slate-500">Pemenuhan Persyaratan Usaha</div>
                                </div>
                                <InputRadio
                                    id="syaratSKK"
                                    isTrue={values.syaratSKK}
